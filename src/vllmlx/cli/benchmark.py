...
@click.option(
    "--iterations", "-n",
    default=5,
    help="Number of iterations per prompt (default: 5)"
)
@click.option(
    "--max-tokens", "-t",
    default=100,
...
@click.option(
    "--prompt", "-p",
    multiple=True,
    help="Custom prompt(s) to use (can specify multiple)"
)
@click.option(
    "--warmup", "-w",
    default=1,
...
@click.option(
    "--verbose", "-v",
    is_flag=True,
    help="Show detailed output for each iteration"
)
@click.option(
    "--skip-cold-start",
    is_flag=True,
...
@click.option(
    "--timeout-load", 
    default=300,
    help="Timeout for model loading in seconds (default: 300)"
)
@click.option(
    "--timeout-gen",
    default=120,
    help="Timeout for generation in seconds (default: 120)"
)
def benchmark(
    model: str,
    iterations: int,
    ...
    console.print(f"   System: {_format_memory(memory_before.system_used_gb)} used ({memory_before.system_percent:.1f}%)")
    ...
    if not skip_cold_start:
        ...
        try:
            with timeout(timeout_load, f"Cold start timed out after {timeout_load}s"):
                cold_start_time = _measure_cold_start(model_path)
    ...
    try:
        with timeout(timeout_load, f"Warm start timed out after {timeout_load}s"):
            warm_start_time, model_obj, processor, model_config = _measure_warm_start(model_path)
    except TimeoutError as e:
        console.print(f"[red]Error: {e}[/red]")
        raise SystemExit(1)
    ...
    console.print(f"   System: {_format_memory(memory_after_load.system_used_gb)} ({memory_after_load.system_percent:.1f}%)")
    console.print()
    ...
    try:
        ...
        results = _run_benchmark(
            model_obj, processor, model_config,
            prompts, iterations, max_tokens, temp, warmup, verbose, timeout_gen
        )
        ...
        summary = _build_summary(
            model_path, cold_start_time, warm_start_time, results,
            memory_before, memory_after_load, memory_peak, model_memory
        )
...
def _measure_cold_start(model_path: str) -> float:
    ...
    try:
        model, processor, config, elapsed = load_model_with_progress(model_path)
...
def _measure_warm_start(model_path: str) -> Tuple[float, Any, Any, Any]:
    ...
    with Progress(
        ...
        for prompt_idx, prompt_text in enumerate(prompts):
            for iter_num in range(iterations):
                progress.update(
                    task, 
                    description=f"Prompt {prompt_idx + 1}/{len(prompts)}, iter {iter_num + 1}/{iterations}"
                )
                ...
                try:
                    # Generate with TTFT measurement and timeout
            ...
            if verbose and iterations > 1:
                ...
                if prompt_results:
                    ...
                    avg_ttft = statistics.mean([r.time_to_first_token_s for r in prompt_results]) * 1000
                    console.print(f"  [cyan]Prompt avg: {avg_tps:.1f} tok/s, TTFT={avg_ttft:.0f}ms[/cyan]\n")
...
def _display_results(summary: BenchmarkSummary, verbose: bool):
    ...
    table.add_row("  Metal Active (loaded)", _format_memory(summary.memory_after_load.metal_active_gb))
    ...
    if summary.total_iterations > 0:
        ...
        table.add_row("  Average Rate", f"[bold green]{summary.avg_tokens_per_sec:.2f} tok/s[/bold green]")
    ...
    console.print(f"  📈 Peak Metal memory: [red]{_format_memory(summary.memory_peak.metal_peak_gb)}[/red]")
    if summary.total_iterations > 0:
        console.print(f"  ⏱️  Time to first token: [yellow]{summary.avg_ttft_s*1000:.0f}ms[/yellow] avg")
        console.print(f"  🚀 Generation rate: [green]{summary.avg_tokens_per_sec:.2f} tokens/sec[/green] avg")
    else:
